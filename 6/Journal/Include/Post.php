...
class Post extends Database
{
    ...
    public function deletePostById($postId)
    {
        ...
        $stmt = $this->conn->prepare($query);
        // Mengembalikan pesan kesalahan jika pernyataan yang telah disiapkan tidak dapat dibuat.
        if (!$stmt) return $stmt->error; 
        // Mengeksekusi query delete menggunakn bindparam dan execute.
        $stmt->bind_param("i", $postId);
        $result = $stmt->execute();
        // Mengembalikan pesan kesalahan MySQLi.
        if (!$result) return $stmt->error; 
        // Mengembalikan Post tidak ditemukan bila tidak ada post yang di delete.
